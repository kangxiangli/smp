...
<el-row :gutter="20">
  ...
  <el-col :span="12"><div class="grid-content bg-purple">{{$t("popups.31")}}：{{form.frecharge_virtual_address}}</div></el-col>
</el-row>
<el-row :gutter="20">
...
//	import zh from '../../resources/zh';
  ...
  export default {
      ...
      loadData(){
       var _this=this;
        var parme_fid= this.fids;
			_this.$http.get(auth.getServerUrl()+api.VIRTUAL_GENERA_DET+"?fid="+parme_fid)
				 .then(res => {
	...
	return row.ftype=="05"?this.$t('script.13'):this.$t('script.14');
		}
 }
 }</script>
