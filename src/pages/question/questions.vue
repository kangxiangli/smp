		...
		<div slot="body">
			<el-table :stripe="true" :data="tableData.rows" border ref="multipleTable" style="width: 100%" v-loading="listLoading" @selection-change="handleSelectionChange">
				...
				<!--<el-table-column prop="fstatus" :formatter="fstatus" :label="$t('table.5')">
					...
					<el-table-column class-name="status-col" prop="fstatus" :label="$t('table.5')" >
					 <template scope="scope">
			          <el-tag :type="scope.row.fstatus | statusFilter" >{{scope.row.fstatus == 0 ?$t('script.1'): $t('script.2')}}</el-tag>
			        </template>
				</el-table-column>
	...
	export default {
		...
		methods: {
			...
			loadData(str, timeStar, timeEnd, ftype) { // ftype;//类型 fuserName;//登录名称
				...
				this.$http.get(auth.getServerUrl() + api.VIRTUAL_FQUESTION_LIST + "?fuserName="+str + "&beginDate=" + timeStar + "&endDate=" + timeEnd + "&ftype=" + ftype+ "&rows=" + this.tableData.pagination.pageSize + "&page=" + this.tableData.pagination.pageNo)
					.then(res => {
